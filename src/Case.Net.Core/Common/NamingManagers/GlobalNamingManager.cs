...
namespace Case.Net.Common.NamingManagers
{
    ...
    {
        private Dictionary<string, INamingConvention> _conventions;

        public GlobalNamingManager() { _conventions = new Dictionary<string, INamingConvention>(); }

        private static IEnumerable<INamingConvention> LoadFromAssembly(Assembly assembly)
        {
            ...
            {
                var convention =
                (INamingConvention) Activator.CreateInstance( type );

                yield return convention;
            }
        }

        public IEnumerator<INamingConvention> GetEnumerator()
        {
